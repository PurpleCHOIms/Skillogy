...
!`if curl -s -o /dev/null -m 2 http://localhost:7474; then COUNT=$(curl -s -u "${NEO4J_USER:-neo4j}:${NEO4J_PASSWORD:-skillogy}" -H "Content-Type: application/json" -d '{"statements":[{"statement":"MATCH (s:Skill) RETURN count(s) AS n"}]}' http://localhost:7474/db/neo4j/tx/commit 2>/dev/null | sed -n 's/.*"row":\[\([0-9][0-9]*\)\].*/\1/p'); if [ -n "$COUNT" ]; then echo "Neo4j: UP — $COUNT skills indexed"; else echo "Neo4j: UP but query failed (auth?)"; fi; else echo "Neo4j: DOWN"; fi`
